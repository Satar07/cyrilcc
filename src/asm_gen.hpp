  ...
  private:
            ...
            case IROp::LOAD: {
                ...
                if (alloca_map.count(src_name)) {
                    ...
                    throw std::runtime_error(
                        "LOAD source must be an 'alloca' or 'global' variable: " + src_name);
                }
                break;
            }
    ...
    void ensure_in_reg(const IROperand &op, int target_reg) {
        if (std::holds_alternative<int>(op.value)) {
            ...
            emit("LOD R" + std::to_string(target_reg) + ", " +
                     std::to_string(std::get<int>(op.value)),
                 "Load immediate");
            return;
        }
    ...
    void get_var_address(const IROperand &op, int target_reg, int offset = 0) {
        ...
        if (home_offset != temp_home_map.end()) {
            ...
            if (offset != 0) {
                throw std::runtime_error("get_var_address with non-zero offset not fully "
                                         "implemented "
